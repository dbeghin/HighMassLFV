...
int main(/*int argc, char** argv*/) {
  TFile* file_out = new TFile("HighMassLFVMuTau/fakerate_factorised_flavorMtLow.root", "RECREATE");
  ...
  TH1D* hpass_taupt_MC_flavor[half_var][flavor.size()];
  TH1D* hfail_taupt_MC_flavor[half_var][flavor.size()];
  TH1D* hpass_ratio_MC_flavor[half_var][flavor.size()];
  TH1D* hfail_ratio_MC_flavor[half_var][flavor.size()];
  double integral_pass_flavor[half_var][flavor.size()];
  double integral_fail_flavor[half_var][flavor.size()];
  ...
  for (unsigned int k=0; k<vars.size(); ++k) {
	...
	if (k%2 == 0) hpass_taupt_MC_total[half_k] = (TH1D*) hpass_taupt_MC[half_k][i]->Clone("FakeRateByTauPt_total");
	if (k%2 != 0) hfail_ratio_MC_total[half_k] = (TH1D*) hfail_ratio_MC[half_k][i]->Clone("den3_total");
	if (k%2 == 0) hpass_ratio_MC_total[half_k] = (TH1D*) hpass_ratio_MC[half_k][i]->Clone("RatioCorrectionFactor_total");
	...
	if (k%2 == 0) integral_pass_total[half_k] += hpass_taupt_MC[half_k][i]->Integral();
      ...
      if (m==0 && dd == 0) {
	if (k%2 != 0) hfail_taupt_MC_flavor[half_k][l] = (TH1D*) hfail_taupt_MC[half_k][i]->Clone("den2_flavor_"+flavor[l]);
	if (k%2 == 0) hpass_taupt_MC_flavor[half_k][l] = (TH1D*) hpass_taupt_MC[half_k][i]->Clone("FakeRateByTauPt_flavor_"+flavor[l]);
	if (k%2 != 0) hfail_ratio_MC_flavor[half_k][l] = (TH1D*) hfail_ratio_MC[half_k][i]->Clone("den3_flavor_"+flavor[l]);
	if (k%2 == 0) hpass_ratio_MC_flavor[half_k][l] = (TH1D*) hpass_ratio_MC[half_k][i]->Clone("RatioCorrectionFactor_flavor_"+flavor[l]);
	if (k%2 != 0) integral_fail_flavor[half_k][l] = hfail_taupt_MC[half_k][i]->Integral();
	if (k%2 == 0) integral_pass_flavor[half_k][l] = hpass_taupt_MC[half_k][i]->Integral();
      }
      else {
	if (k%2 != 0) hfail_taupt_MC_flavor[half_k][l]->Add(hfail_taupt_MC[half_k][i]);
	if (k%2 == 0) hpass_taupt_MC_flavor[half_k][l]->Add(hpass_taupt_MC[half_k][i]);
	if (k%2 != 0) hfail_ratio_MC_flavor[half_k][l]->Add(hfail_ratio_MC[half_k][i]);
	if (k%2 == 0) hpass_ratio_MC_flavor[half_k][l]->Add(hpass_ratio_MC[half_k][i]);
	if (k%2 != 0) integral_fail_flavor[half_k][l] += hfail_taupt_MC[half_k][i]->Integral();
	if (k%2 == 0) integral_pass_flavor[half_k][l] += hpass_taupt_MC[half_k][i]->Integral();
      }
  ...
  for (unsigned int half_k=0; half_k<half_var; ++half_k) {
    ...
    for (unsigned int l=0; l<flavor.size(); ++l) {
      hpass_taupt_MC_flavor[half_k][l]->Divide(hfail_taupt_MC_flavor[half_k][l]);
      hpass_ratio_MC_flavor[half_k][l]->Divide(hfail_ratio_MC_flavor[half_k][l]);
      double av_fr = integral_pass_flavor[half_k][l]/integral_fail_flavor[half_k][l];
      cout << av_fr << endl;
      hpass_ratio_MC_flavor[half_k][l]->Scale(1./av_fr);
    }
    for (unsigned int dd=0; dd<DM.size(); ++dd) {
      for (unsigned int m=0; m<eta.size(); ++m) {
  ...
  for (unsigned int half_k=0; half_k<half_var; ++half_k) {
    ...
    for (unsigned int l=0; l<flavor.size(); ++l) {
      hpass_taupt_MC_flavor[half_k][l]->Write();
      hpass_ratio_MC_flavor[half_k][l]->Write();
    }
    for (unsigned int dd=0; dd<DM.size(); ++dd) {
      for (unsigned int m=0; m<eta.size(); ++m) {
