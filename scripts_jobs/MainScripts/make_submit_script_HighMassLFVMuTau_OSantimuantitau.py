...
if __name__ == "__main__":
    ...
    region = "CR4"
    folder = "HighMassLFVMuTau/OSantimuantitau_CR4"
    ...
    for jj in range(0, len(pnfn)):    
        ...
        for i in f.readlines():   #f contains the root files in the /pnfs directory
            ...
            if ligne%files_per_job==0:   
                ...
                command1 = command1 + "cp /user/dbeghin/Work/Taus/Reweighting/*.root Reweighting/" + "\n"
                outFile.write(command1)
                command3 = "qsub -q localgrid@cream02 -o "+ "../out_err/test"+str(ligne/files_per_job)+myname[jj] +".stdout -e " +"../out_err/test"+str(ligne/files_per_job)+myname[jj] +".stderr -l walltime=15:00:00    " + "../Jobs_to_submit/test"+str(ligne/files_per_job)+name_out + "\n"
                submit_File.write(command3)
