...
if __name__ == "__main__":
    ...
    region = "CR2"
    folder = "HighMassLFVMuTau/OSisomuantitau_CR2"
    ...
    for jj in range(0, len(pnfn)):    
        ...
        for i in f.readlines():   #f contains the root files in the /pnfs directory
            ...
            if ligne%files_per_job==0:   
                ...
                command1 = command1 + "cp /user/dbeghin/Work/Taus/Reweighting/*.root Reweighting/" + "\n"
                outFile.write(command1)
                command3 = "qsub -q localgrid@cream02 -o "+ "../out_err/test"+str(ligne/files_per_job)+myname[jj] +".stdout -e " +"../out_err/test"+str(ligne/files_per_job)+myname[jj] +".stderr -l walltime=30:00:00    " + "../Jobs_to_submit/test"+str(ligne/files_per_job)+name_out + "\n"
                submit_File.write(command3)
            ...
            #command2 = command2 + " \n"+ "cp  Outout" +str(ligne)+myname[jj]+ ".root \t" + "/user/dbeghin/Work/Taus/"+folder+"/Out_"+myname[jj]+"/Out" +str(ligne)+myname[jj]+ ".root"
            command2 = command2 + "\n rm " + bare_fname
            command2 = command2 + " \n\n\n"
