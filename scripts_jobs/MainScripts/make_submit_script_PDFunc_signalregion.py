...
if __name__ == "__main__":
    ...
    for jj in range(0, len(pnfn)):    
        ...
        command1 = command1 + "mkdir /user/dbeghin/Work/MuTauHighMass/"+folder+"/Out_PDFunc_"+myname[jj] + "\n"
        ...
        for i in f.readlines():   #f contains the root files in the /pnfs directory
            ...
            if ligne%files_per_job==0:   
                ...
                outFile.write("cp  "+con_name+" \t" + "/user/dbeghin/Work/MuTauHighMass/"+folder+"/Out_PDFunc_"+myname[jj]+"/"+con_name+"\n")
                outFile.write("rm -f *.root\n")
                outFile.close()
                scr_name="test"+str(con_nbr)+myname[jj]
                outFile = open("../Jobs_to_submit/"+scr_name+".sh" , 'w')
                command1 = "mkdir /user/dbeghin/Work/MuTauHighMass/"+folder+"/Out_PDFunc_"+myname[jj] + "\n"
            ...
            #command2 = command2 + " \n"+ "cp  Out" +str(ligne)+myname[jj]+ ".root \t" + "/user/dbeghin/Work/MuTauHighMass/"+folder+"/Out_PDFunc_"+myname[jj]+"/Out" +str(ligne)+myname[jj]+ ".root"
            command2 = command2 + "\n rm " + bare_fname
            command2 = command2 + " \n\n\n"
