...
if __name__ == "__main__":
    ...
    for jj in range(0, len(pnfn)):    
        ...
        f=os.popen("ls -t " + pnfn[jj] + "outfile* | sort")   #you may want to replace "outfile" with the name of the root files found in the /pnfs folder
        ...
        command1 = command1 + "mkdir -p $workdir/Skims_highmass/Out_"+myname[jj] + "\n"      #Make output directories
        command1 = command1 + "cd $workdir"
        outFile.write(command1)
        command3 = "qsub -q localgrid@cream02 -o " + "../out_err/"+myname[jj] +".stdout -e " +"../out_err/"+myname[jj] +".stderr -l walltime=50:00:00    " + "../Jobs_to_submit/"+name_out + "\n"  #Command to submit one job to the localgrid
        ...
        for i in f.readlines():   #f contains the root files in the /pnfs directory
            ...
            if ligne%files_per_job==0:   
                ...
                command1 = command1 + "cd $workdir"
                outFile.write(command1)
                command3 = "qsub -q localgrid@cream02 -o "+ "../out_err/test"+str(ligne/files_per_job)+myname[jj] +".stdout -e " +"../out_err/test"+str(ligne/files_per_job)+myname[jj] +".stderr -l walltime=50:00:00    " + "../Jobs_to_submit/test"+str(ligne/files_per_job)+name_out + "\n"
                submit_File.write(command3)
            ...
            command2 = "\n" + "./" + code_name + ".exe " + "$scratchdir/Out" +str(ligne)+myname[jj]+ ".root dcap://maite.iihe.ac.be" +   i[0:-1] + " stuff " + myoption[jj]
            command2 = command2 + " \n"+ " mv  $scratchdir/Out" +str(ligne)+myname[jj]+ ".root \t" + "$workdir/Skims_highmass/Out_"+myname[jj]
            command2 = command2 + " \n\n\n"
